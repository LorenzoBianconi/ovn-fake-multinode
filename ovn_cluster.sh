...
CENTRAL_NAME="${CENTRAL_NAME:-ovn-central-az1}"
CENTRAL_NAMES=()
...
EOF
    ...
    if [ "$ovn_central" == "yes" ]; then
        for name in "${GW_NAMES[@]}"; do
            ...
            [ $CENTRAL_COUNT -gt 1 ] && is_ic_gw="yes" || is_ic_gw="no"
            ${RUNC_CMD} exec ${name} bash /data/configure_ovn.sh eth1 \
                ${ovn_remote_gw} is_gw ${ovn_monitor_all} ${ovn_dp_type} ${is_ic_gw}
...
EOF
    ...
    if [ $CENTRAL_COUNT -gt 1 -a $az -eq 1 ]; then
